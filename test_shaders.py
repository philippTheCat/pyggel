...
def main():
    ...
    my_scene.add_3d_shader(obj) #some new shader objects, eh?
    my_scene.add_3d_shader(obj2)
    my_scene.add_3d_shader(box)
    for i in img3d:
        my_scene.add_3d(i)
    ...
    if pyggel.shaders.usable:
        my_shader = pyggel.shaders.Shader(
            '''
            // Vertex program
            varying vec3 normal;
            void main() {
            ...
            void main() {
                ...
                // quantize to 3 steps
                intensity = (floor(dot(l, n) * 3.0)+1)/3.0;
                
                color = vec4(intensity*1+.2, intensity*1+.2, intensity*1+.2, 1.0);
         
                gl_FragColor = color;
            }
            ''') #a simple toon shader
        my_scene.set_shader(my_shader)
